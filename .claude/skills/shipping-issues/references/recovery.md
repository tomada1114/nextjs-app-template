...
fail the baseline on its own. Seen in practice — a `.pnpm-store/` holding a unix socket,
which a test helper that copies untracked files hit with a bare `ENOTSUP`. This is why
