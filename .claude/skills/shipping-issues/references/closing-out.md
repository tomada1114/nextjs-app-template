...
(`holding/42/packages/legacy-cli/`) so the report can name what came from where; on a
name collision add a suffix rather than overwrite. A move across filesystems is a
copy-then-delete — fine for a fixture, slow for `node_modules`; keep holding for what
