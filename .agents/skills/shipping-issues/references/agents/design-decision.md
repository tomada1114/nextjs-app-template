...
the user. `LABEL: left-on` with `VERDICT: DECIDED` means only the label write failed:
clear it from this session before treating the issue as ready. `APPROACH` is the only
part worth reading closely in this context — the full design lives on the issue, where
