...
describe("paths: checkRead", () => {
  ...
  it("does not block a settings.local.json outside .claude/", () => {
    // The rule is anchored to the exact repository-root path `.gitignore`
    // carries, not to the basename alone — a nested or differently-placed
    // file of the same name is not this rule's concern.
